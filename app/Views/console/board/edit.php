...
<input type="hidden" id="main_file_hidden" name="main_file_hidden" value="<?= $info->main_file_id ?>">
...
<?php if ($board_config->category_yn == 'Y') { ?>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="category" name="category" placeholder="카테고리를 입력하세요" value="<?= $info->category ?>">
                </div>
<?php } ?>
                ...
                <!-- 대표 이미지 -->
                <div class="mb-3">
                    <label for="main_file" class="form-label">대표 이미지</label>
                    <input type="file" class="form-control" id="main_file" name="main_file" onchange="uploadFile(this.id, 'image', 'uploadMainFileAfter')">
                    <div class="mb-2 mt-2" id="main_file_view">
<?php   if ($info->main_file_info != null) { ?>
                        <div class="row g-2 align-items-center">
                            <div class="col">
                                <img src="/csl/file/view/<?= $info->main_file_id ?>" class="img-thumbnail" style="width: 300px; height: auto;">
                            </div>
                            <div class="col">
                                <small class="text-muted">원본파일명</small><br>
                                <?= $info->main_file_info->file_name_org ?>
                            </div>
                            <div class="col">
                                <small class="text-muted">사이즈</small><br>
                                <?= $info->main_file_info->file_size_kb ?>KB
                            </div>
                        </div>
<?php   } ?>
                    </div>
...
<?php if ($board_config->pdf_yn == 'Y') { ?>
                ...
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" id="pdf_file" name="pdf_file" onchange="uploadFile(this.id, 'file', 'uploadPdfFileAfter')">
                    <div class="mb-2 mt-2" id="pdf_file_view">
...
<?php if ($board_config->reg_date_yn == 'Y') { ?>
                <!-- 등록일 -->
                <div class="mb-3">
                    <label for="reg_date" class="form-label">게시일</label>
...
<?php if ($info->board_idx > 0) { ?>
                    <a href="/csl/board/view/<?= $info->board_id ?>/<?= $info->board_idx ?>" class="btn btn-secondary">취소</a>
<?php } else { ?>
                    <a href="/csl/board/list?board_id=<?= $info->board_id ?>" class="btn btn-secondary">취소</a>
<?php } ?>
...
<?php if ($board_config->reg_date_yn == 'Y') { ?>
        $("#reg_date").inputmask("datetime", {inputFormat:"yyyy-mm-dd"});
<?php } ?>
    });

    function boardUpdate() {
        ajax1('/csl/board/update', 'frm', 'boardUpdateAfter');
    }
    ...
    function uploadMainFileAfter(proc_result) {
        var result = proc_result.result;
        var message = proc_result.message;
        var info = proc_result.info;
        if (result == true) {
            $('#main_file_hidden').val(info.file_id);
