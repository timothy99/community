...
<?php   } ?>
                    ...
                    <div class="row g-0 border-bottom">
                        <div class="tbl-label">수정자</div>
                        <div class="tbl-value"><?= $info->upd_id ?></div>
                    </div>
                    <div class="row g-0 border-bottom">
            ...
            <div class="card-footer text-end">
                <div class="d-flex gap-2 justify-content-end">
                    ...
                    <a href="/csl/board/<?= $info->board_id ?>/list<?= !empty($http_query) ? '?'.$http_query : '' ?>" class="btn btn-secondary">목록</a>
                    <a href="/csl/board/<?= $info->board_id ?>/edit/<?= $info->board_no ?><?= !empty($http_query) ? '?'.$http_query : '' ?>" class="btn btn-primary">수정</a>
                </div>
            </div>
        </div>
...
<?php       } ?>
                ...
                <!-- 댓글 작성 -->
                <div class="mt-4" id="comment-editor-root">
...
<script>
    ...
    function commentInsert() {
        ...
        ajax1('/csl/comment/insert', update_form, 'commentAfter');
    }
    ...
    function commentEdit(board_comment_idx) {
        ...
        ajax1('/csl/comment/edit/'+board_comment_idx, 'frm', 'commentEditAfter');
    }
    ...
    function commentEditAfter(proc_result) {
        ...
        var comment = proc_result.comment || '';
        var file_list = proc_result.file_list || [];
        activateCommentEditMode(board_comment_idx, comment, file_list);
    }

    function commentUpdate(board_comment_idx) {
        var update_form = new FormData();
        var comment = getCommentEditorCode();
        update_form.append('board_comment_idx', board_comment_idx);
        update_form.append('comment', comment);
        update_form.append('file_idxs', getCommentFileIds());
        ajax1('/csl/comment/update', update_form, 'commentAfter');
    }
    ...
    function commentDelete(board_comment_idx) {
        ...
        if(confirm('댓글을 삭제하나요? 삭제하면 복구가 불가능합니다.')) {
            var update_form = new FormData();
            update_form.append('board_comment_idx', board_comment_idx);
            ajax1('/csl/comment/delete', update_form, 'commentAfter');
        }
    }
    ...
    function activateCommentEditMode(board_comment_idx, comment, file_list) {
    ...
    function resetCommentEditor() {
        $('#comment-editor-root').show().append($('#comment-editor-panel'));
        $('.comment-display').show();
        $('.comment-edit-host').addClass('d-none').empty();
        $('.tbl-action').show();
