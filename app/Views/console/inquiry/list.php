        ...
        <div class="card mb-4">
            ...
            <div class="card-footer text-end">
                <div class="d-flex gap-2 justify-content-end">
                    ...
                    <a href="/csl/inquiry/list" class="btn btn-secondary">초기화</a>
                </div>
            </div>
        </div>
        <!-- 목록 -->
...
<?php   foreach($list as $no => $val) { ?>
                            <tr>
                                <td><?=$val->list_no ?></td>
                                <td><a href="/csl/inquiry/view/<?=$val->inquiry_idx ?>"><?=$val->name ?></a></td>
                                <td><?=$val->phone ?></td>
                                <td><?=$val->email ?></td>
                                <td class="text-start"><?= mb_strimwidth($val->contents, 0, 50, '...') ?></td>
...
<script>
    ...
    function search() {
        ...
        location.href = '/csl/inquiry/list?search_page='+search_page+'&search_text='+search_text+'&search_condition='+search_condition+'&search_rows='+search_rows;
    }
</script>
